...
Modern logistics networks increasingly demand in efficient route planning
...
of points of interest (POIs) within a limited time budget.
...
In this paper, we consider a graph-based version of the TTSP. The input is a
weighted graph with a fixed depot and a reward value assigned to each candidate
location, together with a hard upper bound~$L$ on the total length of the tour.
The goal is to construct a closed tour that starts and ends at the depot,
visits only a subset of the available locations, and maximizes the collected
reward without exceeding the travel budget. Rather than focusing on an exact
optimization formulation, this report studies heuristic approaches for this
problem: a greedy insertion baseline, local-search improvements, and a genetic
algorithm. The contribution of the work is an empirical comparison of these
heuristic design choices for graph-based TTSP instances.
...
The literature also contains a wide range of heuristic approaches for these
problems, since exact optimization quickly becomes computationally expensive on
larger instances~\cite{Vansteenwegen2011}. This have motivated the empirical comparison in this report, which
studies how different heuristic design choices perform on graph-based TTSP
instances.
...
The contribution of this thesis is an empirical study of heuristic design
choices for the TTSP. The study focuses on how increasing heuristic complexity 
affects the balance between collected reward and computational runtime.

In particular, this thesis makes the following contributions:
\begin{itemize}
    \item It compares three heuristic approaches for the TTSP: a greedy
    insertion baseline, a local-search heuristic using prize-aware
    add/drop/swap moves together with 2-opt route improvement, and a memetic
    genetic algorithm that combines population-based search with local
    improvement.
    \item It evaluates how these heuristic choices trade solution quality
    against runtime across different instance sizes and travel budgets, with
    the goal of identifying when the additional cost of more complex heuristics
    is justified.
\end{itemize}
