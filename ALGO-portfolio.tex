...
undirected planar graph topologies, after which random integer edge weights are
assigned to the edges. Since Rudy does not generate rewards for the TTSP, each
instance is augmented with one integer reward for every vertex. The depot is
still treated separately by the algorithms, so the reward written for the depot
is ignored when computing the collected reward.
...
the greedy baseline reaches the optimum on 84 of them. With travel budget 100,
the GA again reaches the exact optimum on all 100 small instances, while the
greedy baseline reaches it on 94 of them. On medium instances, the GA provides
the clearest improvement: it wins on 79 of 100 instances and improves the mean
reward ratio from 0.967 to 1.000 relative to the best-known reward. On large
instances, the improvement is smaller: the GA wins on 43 of 100 instances and
improves the mean reward ratio from 0.993 to 1.000.
Rows for travel budgets 200, 300, and 500 currently have matching GA result
sets for medium instances. Large instances also have matching GA result sets for
travel budget 200. Other small and large rows for those budgets report only the
greedy baseline, so their reward-ratio entries are left blank until matching GA
result sets are available for the same problem instances and travel budgets.
...
\hline
Greedy Baseline & 100 & 312.3 &  & 0.289 & 0.001 \\
\hline
\multicolumn{6}{|l|}{$L_{\max}=300$} \\
\hline
Greedy Baseline & 100 & 312.3 &  & 0.193 & 0.001 \\
\hline
\multicolumn{6}{|l|}{$L_{\max}=500$} \\
\hline
Greedy Baseline & 100 & 312.3 &  & 0.116 & 0.001 \\
\hline
\end{tabular}
...
\multicolumn{6}{|l|}{$L_{\max}=300$} \\
\hline
Greedy Baseline & 100 & 6047.4 &  & 0.999 & 17.749 \\
\hline
\multicolumn{6}{|l|}{$L_{\max}=500$} \\
\hline
Greedy Baseline & 100 & 9049.2 &  & 0.999 & 45.382 \\
\hline
\end{tabular}
...
useful on medium instances, where it improves the mean reward ratio and beats the
greedy baseline on most instances. On large instances, the current GA
configuration gives only a small average reward improvement over greedy while
