...
optimal reward for the TTSP. This solver is not intended as the main scalable
method. Instead, it provides a reference point for assessing the quality of
heuristic solutions on instances where exact optimization is computationally
feasible. The solver is implemented as a constraint-programming model using
Choco Solver
...
The advantage of this approach is that it explores many possible construction
orders instead of committing to the single locally best sequence chosen by the
greedy baseline. Some priority orders may insert a node earlier or later than
the greedy baseline would, which can change the feasible insertion positions
available later in the construction. This allows the genetic algorithm to find
tours that the single greedy construction may miss.
...
individual evaluation decodes a priority-order chromosome into a feasible tour.
The decoder uses the same insertion routine as the greedy baseline, except that
it considers nodes in chromosome order instead of always choosing the globally
best immediate insertion. In the worst case, decoding one chromosome can
consider $O(n^2)$ insertions, with reward accounting costs similar to the greedy
implementation. The total running time is therefore dominated by evaluating the
initial population and all generated offspring over $G$ generations.
\\ \\
