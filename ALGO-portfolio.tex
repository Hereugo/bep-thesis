...
% associated reward value, but the depot reward is not collected. The input is an
...
$d(a,v)+d(v,b)-d(a,b)$, where each distance is a shortest-path distance in the
input graph. Insertions that would exceed the budget~$L$ are discarded.

The reward gain of an insertion is not necessarily only the reward of node~$v$.
Because each connection in the tour is expanded into a shortest path in the
original graph, inserting $v$ may also add or remove intermediate nodes from the
expanded route. The implementation therefore evaluates the total collected
reward after the insertion, counting each visited node at most once and excluding
the depot, and uses the difference from the current collected reward as the
reward gain. Each feasible insertion is scored by
\[
  \frac{\text{reward gain}}{\text{additional travel cost}}.
\]
...
with positive reward gain remains.
